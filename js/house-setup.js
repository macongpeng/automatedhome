...
JAHOME.initFeatureData = function (file, dataTypes) {
	'use strict';

	$.get(file, function (data) {
		JAHOME.featureData = data['feature-types'];
...
// since we are modifying the JAHOME namespace AND using "use strict", we have to return JAHOME.
// passing in JAHOME for robustness.
JAHOME.initHomeData = function (file, dataTypes) {
	'use strict';

	$.getJSON(file, function (data) {

		JAHOME.homeData = data['whitehouse'];
	}, 'json');

  // Load the temp set previously for White House
	$.getJSON(JAHOME.setFeatureURL).done(function (data) {
    JAHOME.homeData.features.temp['current-value'] = data.temp
		JAHOME.initControlPanel('whitehouse', 'control-panel-content');
	}, 'json');
...
JAHOME.initControlPanel = function (homeID, containerID) {
    ...
    $.each(JAHOME.homeData.rooms, function (index, room) {
        var domID = homeID + JAHOME.idDelimiter + index;

        // use a div & h3 heading for styling
        roomHTML += '<div id="' + domID + '" class="room"><h3>' + room.name + '</h3>';
        roomHTML += JAHOME.featureControlsHTML(JAHOME.featureTypes, room.features, domID);
        roomHTML += '</div>';
    });
...
JAHOME.initFeatureTypes = function (featureTypeData, featureTypeDataTypes) {
	...
	$.each(featureTypeData, function (index, feature) {
		featureTypeData[index]['data-type-object'] = featureTypeDataTypes[feature['data-type']];
	});
